...
def test_update_local_tables_origianal(parse_original_parcel_data) -> list[dict]:
    update_local_tables.owners(
        parse_original_parcel_data[0], db_name=test_debian_dbname, db_uri=test_debian_uri
    )
    update_local_tables.rentals(
        parse_original_parcel_data[1], db_name=test_debian_dbname, db_uri=test_debian_uri
    )
